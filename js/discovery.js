...
function updateMatchRangeSummary() {
  if (matchSource === 'live' && !matchSourceStream) {
    matchRangeSummary.textContent = 'NerdSync will use your current live viewer count and category.';
    return;
  }
...
async function loadMatchVods() {
  matchVodEl.innerHTML = '<option value="">Loading past broadcasts…</option>';
  try {
    const videos = await fetchVideosForBroadcaster(currentUser.id, currentToken, 20);
    matchVodEl.innerHTML = '<option value="">Choose a past broadcast</option>' + videos.map(video => `<option value="${escapeHtml(video.id)}">${escapeHtml(new Date(video.created_at).toLocaleDateString())} · ${escapeHtml(video.title || 'Untitled broadcast')}</option>`).join('');
...
async function loadCreatorMatches(options = {}) {
  ...
  if (matchSource === 'live') {
    ...
    if (!matchSourceStream) {
      matchRangeSummary.textContent = 'Your channel is offline. Choose Enter a past peak or Past VOD + entered peak.';
      return [];
    }
    matchPeak = matchSourceStream.viewer_count;
  } else matchPeak = numOrNull(matchPeakEl.value);
  const range = matchAudienceRange();
  if (!range) {
    matchRangeSummary.textContent = 'Enter a valid past peak of at least one viewer.';
    return [];
  }
  updateMatchRangeSummary();

  const categoryMap = new Map();
  filters.categories.slice(0, MAX_TOP_CATEGORIES_FOR_DISCOVER).forEach(category => categoryMap.set(category.id, category));
  recommendationCategorySeeds(MAX_TOP_CATEGORIES_FOR_DISCOVER).forEach(category => {
    if (categoryMap.size < MAX_TOP_CATEGORIES_FOR_DISCOVER && !categoryMap.has(category.id)) categoryMap.set(category.id, category);
  });
  if (matchSourceStream?.game_id && categoryMap.size < MAX_TOP_CATEGORIES_FOR_DISCOVER) categoryMap.set(matchSourceStream.game_id, { id:matchSourceStream.game_id, name:matchSourceStream.game_name });
  if (!categoryMap.size) (await fetchTopGames(currentToken, MAX_TOP_CATEGORIES_FOR_DISCOVER)).forEach(game => categoryMap.set(game.id, game));
  const games = [...categoryMap.values()];
  diagnostics.categories = games.length;
  const perCategory = await Promise.allSettled(games.map(async game => {
    ...
    return streams
      ...
      .map(stream => {
        const distance = Math.abs(stream.viewer_count - matchPeak) / Math.max(matchPeak, 1);
        return { ...stream, _matchDistance:distance, _via:game.name, _why:`Audience match · ${Math.round(distance * 100)}% from ${matchPeak} · ${stream.viewer_count} live viewers` };
      })
      .sort((a,b) => a._matchDistance - b._matchDistance)
      .slice(0, STREAMS_PER_CATEGORY_DISCOVER);
  }));
  diagnostics.failures += perCategory.filter(result => result.status === 'rejected').length;
  const merged = new Map();
  perCategory.forEach(result => {
    if (result.status === 'fulfilled') result.value.forEach(stream => {
      if (!merged.has(stream.user_id) || stream._matchDistance < merged.get(stream.user_id)._matchDistance) merged.set(stream.user_id, stream);
    });
  });
